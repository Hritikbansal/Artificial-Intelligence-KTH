...
//                System.out.println("j"+j+"Sum: "+Sum);
    ...
    ArrayList<ArrayList<Double>> LastAlpha ;
    Integer NewIterStart ;
...
//            ////System.out.println(GammaSumObsSel);
     ...
     public ArrayList<Double> PredictNextObservation (ArrayList<Integer> ObsSeq){
        LastAlpha = GetEmissionSequenceProbIter(ObsSeq) ;
...
//        System.err.println(Temp);
    ...
    public ArrayList<ArrayList<Double>> GetEmissionSequenceProbIter(ArrayList<Integer> ObsSeq){
//        System.err.println("Inside EmissionIter and NewStartIter = "+ NewIterStart);
        if(ObsSeq.isEmpty()){
            throw new IllegalArgumentException("No observations found") ;
        }
        if(LastAlpha == null || NewIterStart == 0){
        LastAlpha = GetAlphaInit(ObsSeq.get(0)) ;
        NewIterStart = 1 ;
        }
        int i;
        for( i=NewIterStart ; i<ObsSeq.size() ; i++){
            LastAlpha = GetAlpha(LastAlpha,ObsSeq.get(i)) ;
        }
        NewIterStart=i ;
